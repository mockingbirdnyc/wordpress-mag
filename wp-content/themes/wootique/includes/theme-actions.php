...
if (!function_exists( 'woo_custom_styling')) {
	function woo_custom_styling() {
		...
		$output = '';
		// Get options
		$background_color = wootique_get_woo_option( 'woo_background_color' );
		$body_img = wootique_get_woo_option( 'woo_body_img' );
		$menu_opacity = wootique_get_woo_option( 'woo_menu_opacity' );
		$container_shadow = wootique_get_woo_option( 'woo_container_shadow' );
		$body_repeat = wootique_get_woo_option( 'woo_body_repeat' );
		$body_position = wootique_get_woo_option( 'woo_body_pos' );
		$body_attachment = wootique_get_woo_option( 'woo_body_attachment' );
		$link = wootique_get_woo_option( 'woo_link_color' );
		$hover = wootique_get_woo_option( 'woo_link_hover_color' );
		//$button = wootique_get_woo_option( 'woo_button_color' );
...
if (!function_exists( 'woo_custom_typography')) {
	function woo_custom_typography() {
		...
		if ( wootique_get_woo_option( 'woo_texttitle' ) == "true" ) {
			...
			//if ( wootique_get_woo_option( 'woo_font_site_title' ) )
				//$output .= '#logo .site-title a {'.woo_generate_font_css( wootique_get_woo_option( 'woo_font_site_title' ) ).'}' . "\n";
			//if ( wootique_get_woo_option('woo_tagline' ) == "true" AND wootique_get_woo_option( 'woo_font_tagline' ) )
				//$output .= '#logo .site-description {'.woo_generate_font_css( wootique_get_woo_option( 'woo_font_tagline' ) ).'}' . "\n";
		}

		if ( wootique_get_woo_option( 'woo_typography' ) == "true") {

			$woo_font_body = wootique_get_woo_option( 'woo_font_body' );
			if ( $woo_font_body )
				$output .= 'body { '.woo_generate_font_css( $woo_font_body, '1.5').' }' . "\n";

			$woo_font_nav = wootique_get_woo_option( 'woo_font_nav' );
			if ( $woo_font_nav )
				$output .= '#navigation, #navigation .nav a { '.woo_generate_font_css( $woo_font_nav, '1.4').' }' . "\n";

			$woo_font_post_title = wootique_get_woo_option( 'woo_font_post_title' );
			if ( $woo_font_post_title )
				$output .= '.post .title { '.woo_generate_font_css( $woo_font_post_title ).' }' . "\n";

			// $woo_font_post_meta = wootique_get_woo_option( 'woo_font_post_meta' );
			//if ( $woo_font_post_meta )
			//	$output .= '.post-meta { '.woo_generate_font_css( $woo_font_post_meta ).' }' . "\n";

			$woo_font_post_entry = wootique_get_woo_option( 'woo_font_post_entry' );
			if ( $woo_font_post_entry )
				$output .= '.entry, .entry p { '.woo_generate_font_css( $woo_font_post_entry, '1.5').' } h1, h2, h3, h4, h5, h6 { font-family: '.stripslashes( $woo_font_post_entry['face']).', arial, sans-serif; }'  . "\n";

			$woo_font_widget_titles = wootique_get_woo_option( 'woo_font_widget_titles' );
			if ( $woo_font_widget_titles )
				$output .= '.widget h3 { '.woo_generate_font_css( $woo_font_widget_titles ).' }'  . "\n";

		// Add default typography Google Font
		} else {

			$woo_just_face = array('face' => 'Varela Round');
			$woo_options['woo_just_face'] = $woo_just_face;
			$output .= 'body, h1, h2, h3, h4, h5, h6, .widget h3, .post .title, .section .post .title, .archive_header, .entry, .entry p, .post-meta { '.woo_generate_font_css( $woo_just_face ).' }' . "\n";
			$output .= '.feedback blockquote p, #post-entries, #breadcrumbs { '.woo_generate_font_css( $woo_just_face ).' }' . "\n";
...
if (!function_exists( 'woo_layout_body_class')) {
	function woo_layout_body_class($classes) {
		...
		global $woo_options;
		$layout = wootique_get_woo_option( 'woo_site_layout' );
...
function woo_feedburner_link ( $output, $feed = null ) {
	...
	if ( wootique_get_woo_option( 'woo_feed_url' ) && ( $feed == $default ) && ( ! stristr( $output, 'comments' ) ) ) $output = esc_url( wootique_get_woo_option( 'woo_feed_url' ) );
